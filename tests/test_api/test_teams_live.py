...
class TestTeamsLive:
    ...
    def test_1_create_team_success(self):
        ...
        response = requests.post(
            f'{BASE_URL}/teams',
            headers=self.headers,
            json={'name': team_name, 'description': 'New team description'},
        )
    ...
    def test_2_create_team_invalid_name(self):
        ...
        response = requests.post(
            f'{BASE_URL}/teams', headers=self.headers, json={'name': 'A'}
        )
        assert response.status_code == 422

        # Слишком длинное имя - 422
        response = requests.post(
            f'{BASE_URL}/teams', headers=self.headers, json={'name': 'A' * 101}
        )
        assert response.status_code == 422

    def test_3_create_team_unauthorized(self):
        """Создание команды без авторизации"""
        response = requests.post(f'{BASE_URL}/teams', json={'name': 'Test Team'})
    ...
    def test_4_create_team_duplicate_name(self):
        ...
        response1 = requests.post(
            f'{BASE_URL}/teams', headers=self.headers, json={'name': team_name}
        )
    ...
    def test_5_get_my_teams(self):
        ...
        response = requests.get(f'{BASE_URL}/teams', headers=self.headers)
    ...
    def test_6_get_team_by_slug_success(self):
        ...
        response = requests.get(
            f'{BASE_URL}/teams/{self.team_slug}', headers=self.headers
        )
    ...
    def test_7_get_team_by_slug_not_found(self):
        ...
        response = requests.get(
            f'{BASE_URL}/teams/non-existent-team-12345', headers=self.headers
        )
    ...
    def test_8_get_team_by_slug_not_member(self):
        ...
        response = requests.get(
            f'{BASE_URL}/teams/{self.team_slug}', headers=other_headers
        )
