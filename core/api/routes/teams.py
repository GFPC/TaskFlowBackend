...
from ...db.models.team import Team, TeamInvitation, TeamMember
from ...db.models.user import User
from ...services.TeamService import TeamService
from ...services.UserService import UserService
from ..deps import get_current_active_user, get_team_service, get_user_service
...
async def create_team(
    ...
    try:
        result = service.create_team(
            name=team_in.name, owner=current_user, description=team_in.description
        )
...
async def get_team_by_slug(
    ...
    try:
        logger.info(
            f'Getting team by slug: {team_slug} for user {current_user.username}'
        )
        ...
        for member in members:
            try:
                logger.debug(
                    f'Serializing member {member.id}, user_id: {member.user_id}'
                )
...
async def add_team_member(
    ...
    try:
        member = service.add_member(
            team=team, user=user, role_name=member_in.role, created_by=current_user
        )
...
async def update_member_role(
    ...
    try:
        member = service.change_member_role(
            team=team, user=user, new_role_name=member_in.role, changed_by=current_user
        )
...
async def transfer_ownership(
    ...
    try:
        result = service.transfer_ownership(
            team=team, new_owner=new_owner, current_owner=current_user
        )
...
async def get_invite_code(
    ...
    try:
        ...
        return InviteCodeResponse(invite_code=code, expires_at=team.invite_code_expires)
    except PermissionError as e:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(e))
...
async def search_teams(
    ...
    teams = service.search_teams(
        query=query, user=current_user, limit=limit, offset=offset
    )
