...
from ..db.models.team import Team, TeamInvitation, TeamMember, TeamMemberRole
...
class TeamService:
    ...
    def _validate_team_name(self, name: str) -> 'Tuple[bool, Optional[str]]':
        ...
        if len(name) < self.TEAM_NAME_MIN_LENGTH:
            return (
                False,
                f'Team name must be at least {self.TEAM_NAME_MIN_LENGTH} characters',
            )

        if len(name) > self.TEAM_NAME_MAX_LENGTH:
            return (
                False,
                f'Team name must be at most {self.TEAM_NAME_MAX_LENGTH} characters',
            )
    ...
    def create_team(
        ...
        member = self.member_model.create(
            team=team, user=owner, role=owner_role, created_by=owner, is_active=True
        )
        ...
        return {'team': team, 'member': member, 'invite_code': team.invite_code}

    # ------------------- Управление участниками -------------------

    def add_member(
        self, team: Team, user: User, role_name: str, created_by: User
    ) -> TeamMember:
        """
        ...
        existing = (
            ...
            .where((self.member_model.team == team) & (self.member_model.user == user))
            .first()
        )
        ...
        # Добавляем участника
        member = self.member_model.create(
            team=team, user=user, role=role, created_by=created_by, is_active=True
        )
        ...
        team.members_count = (
            self.member_model.select()
            .where(
                (self.member_model.team == team) & (self.member_model.is_active == True)
            )
            .count()
        )
        team.save()
    ...
    def remove_member(self, team: Team, user: User, removed_by: User) -> bool:
        """
    ...
    def change_member_role(
        self, team: Team, user: User, new_role_name: str, changed_by: User
    ) -> TeamMember:
        """
    ...
    def transfer_ownership(
        self, team: Team, new_owner: User, current_owner: User
    ) -> Dict[str, Any]:
        """
    ...
    def join_by_code(self, code: str, user: User) -> Dict[str, Any]:
        ...
        if existing:
            if existing.is_active:
                ...
                return {'team': team, 'member': existing}
    ...
    def create_invitation(
        ...
        if invited_user:
            ...
            query = query.where(
                self.invitation_model.invitee_username == invitee_username
            )
        elif invitee_email:
    ...
    def accept_invitation(
        ...
        return {'team': invitation.team, 'member': member}
    ...
    def get_user_teams(self, user: User, active_only: bool = True) -> List[Team]:
        ...
        """
        query = self.member_model.select().where(self.member_model.user == user)
    ...
    def get_team_members(
        ...
        """
        query = self.member_model.select().where(self.member_model.team == team)
    ...
    def get_team_invitations(
        ...
        """
        query = self.invitation_model.select().where(self.invitation_model.team == team)
    ...
    def delete_team(self, team: Team, deleted_by: User) -> bool:
        ...
        self.invitation_model.update(status='cancelled').where(
            (self.invitation_model.team == team)
            & (self.invitation_model.status == 'pending')
        ).execute()
